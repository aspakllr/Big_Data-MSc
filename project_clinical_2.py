# -*- coding: utf-8 -*-
"""
Created on Fri Jan 11 23:00:29 2019
...
y = data['fried']
y=y.astype('int')
X = data.drop(['fried','part_id','weight_loss', 'exhaustion_score', 'gait_speed_slower','grip_strength_abnormal','low_physical_activity'],axis=1)
# Find feature importances with forests of trees
...
for f in range(X.shape[1]):
    i = indices[f]
    print("%d. feature %d - %s (%f) " % (f + 1, indices[f],X.columns[i], importances[indices[f]]))
# Plot the feature importances of the forest
fig = plt.figure()
fig.set_size_inches(18.5, 10.5)
plt.title("Feature importances")
plt.bar(range(X.shape[1]), importances[indices],
       color="mediumvioletred", yerr=std[indices], align="center")
...
# Select x  most important features
features = X.iloc[:,indices[0:30]]

#****************************************************************************************
#labels = data['fried']
#labels=labels.astype('int')
# Remove independent categorical parametres (from chi-test)
#features = data.drop(['fried','part_id','weight_loss', 'exhaustion_score', 'gait_speed_slower','grip_strength_abnormal','low_physical_activity','gender','ortho_hypotension','memory_complain','living_alone','house_suitable_professional','house_suitable_participant','smoking'],axis=1)
...
cv(model, "SVM Classifier",features,labels)
